                            ...
                            @foreach($professors as $professor)
                                @if($professor->status_eletiva == 1)
                                    <div class="col-sm-12 col-md-12 col-lg-3 col-xl-3">
                                        <form action="{{route('students.eletivas.selectProfessor')}}" method="POST">
                                            @csrf
                                            <input type="hidden" name="professor_id" value="{{$professor->id}}">
                                            <input type="hidden" name="student_id" value="{{$student->id}}">
                                            <div class="card">
                                                <div class="card-body" style="min-height: 15rem!important;">
                                                    <div class="card-header" style="font-size: 70%!important;">
                                                        {{$professor->user->name}}
                                                    </div>
                                                    <div class="card-header" style="font-size: 70%!important;">
                                                    ...
                                                    <div class="d-flex flex-column align-items-center text-center">
                                                        ...
                                                        @if($professor->limit_eletiva_students == $professor->students->count())
                                                            ...
                                                            <button class="btn btn-sm btn-secondary w-100" disabled>
                                                                Indisponível [ {{$professor->students->count()}} ] de [ {{$professor->limit_eletiva_students}} ]
                                                            </button>
                                                        @else
                                                            <button type="submit"
                                                                    class="btn btn-sm btn-secondary w-100">
                                                                Vagas Disponíveis:  [ {{$professor->limit_eletiva_students - $professor->students->count()}} ]
