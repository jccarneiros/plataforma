<!-- Modal -->
<div class="modal fade" id="modalQuartoBimestre" data-bs-backdrop="static" tabindex="-1"
     aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <div class="mb-2 text-warning">Turma: {{$discipline->room->name}} / Disciplina: {{$discipline->name}} / Quarto Bimestre</div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            @if ($discipline->a_d_q_b == null)
                <div class="text-center">Digite as aulas previstas e dadas</div>
            @elseif ($discipline->a_d_q_b == '0')
                <div class="text-center">Digite as aulas previstas e dadas</div>
            @else
                <div class="modal-body">
                    <div class="row">
                            ...
                            <table class="table table-sm table-bordered table-hover">
                                ...
                                <tr>
                                    @if($discipline->room->status_q_b !== 1)
                                        <th scope="col">
                                            <input type="text" value="{{$discipline->a_p_q_b}}"
                                                   class="form-control form-control-sm text-center text-uppercase" readonly>
                                        </th>
                                        <th scope="col">
                                            <input type="text" value="{{$discipline->a_d_q_b}}"
                                                   class="form-control form-control-sm text-center text-uppercase" readonly>
                                        </th>
                                    @endif
                                </tr>
                        ...
                        <div class="col-sm-12 col-md-12 col-lg-8 col-xl-8">
                            <form action="{{route('dashboard.students.dados.quartoBimestre',$discipline->id)}}" method="post" enctype="multipart/form-data">
                                @csrf @method('PUT')
                    ...
                    <form action="{{route('dashboard.fechamentos.updateFechamentoStudentsQuartoBimestre',$discipline->id)}}"
                          method="post">
                        @csrf @method('PUT')
                        <div class="modal-body">
                                ...
                                <table class="table table-sm table-bordered table-hover"
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" colspan="3" class="text-center text-uppercase"
                                            style="background-color: #e57373 !important;color: #ffffff !important;">4ºB
                                        </th>
                                        <th scope="col" colspan="3" class="text-center text-uppercase"
                                                    ...
                                                    @switch($fechamento->n_q_b)
                                                        @case(ctype_alpha($fechamento->n_q_b) == true)
                                                            <input type="text" style="width: 2.5rem"
                                                                   ...
                                                                   tabindex="1"
                                                                   value="{{$fechamento->n_q_b,old('n_q_b')}}" required>
                                                            @break
                                                    ...
                                                    @switch($fechamento->n_q_b)
                                                        @case(ctype_alpha($fechamento->n_q_b) == true)
                                                            <input type="text" style="width: 2.5rem"
                                                                   ...
                                                                   tabindex="1"
                                                                   value="{{$fechamento->n_q_b,old('n_q_b')}}" readonly>
                                                            @break
                                                        @case($fechamento->n_q_b >= 5)
                                                            <input type="text" style="width: 2.5rem" name="n_q_b[]"
                                                                   class="table-target text-primary"
                                                                   tabindex="1" value="{{$fechamento->n_q_b,old('n_q_b')}}"
                                                                   readonly>
                                                            @break
                                            ...
                                            <td class="text-center">
                                                @if($discipline->room->status_q_b !== 1)
                                                    <input type="text" style="width: 2.5rem" tabindex="2" step='1'
                                                           ...
                                                           value="{{$fechamento->f_q_b, old('f_q_b')}}" required>
                                                @else
                                                    <input type="text" style="width: 2.5rem" tabindex="2" step='1'
                                                           min="0" max="1000"
                                                           name="f_q_b[]"
                                                           class="table-target text-secondary falta"
                                                           value="{{$fechamento->f_q_b, old('f_q_b')}}" readonly>
                                                @endif
                                            </td>
                                            ...
                                            <td class="text-center">
                                                @if($discipline->room->status_q_b !== 1)
                                                    <input type="text" style="width: 2.5rem" tabindex="3" step='1'
                                                           ...
                                                           value="{{$fechamento->f_c_q_b, old('f_c_q_b')}}" required>
                                                @else
                                                    <input type="text" style="width: 2.5rem" tabindex="3" step='1'
                                                           min="0" max="1000"
                                                           name="f_c_q_b[]"
                                                           class="table-target text-secondary falta_c"
                                                           value="{{$fechamento->f_c_q_b, old('f_c_q_b')}}" readonly>
                                                @endif
                                            </td>
                                            <td>
