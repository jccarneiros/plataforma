                ...
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <label for="filterTipoEnsino" class="text-uppercase">Filtrar por Tipo de Ensino - Total:
                        @if(isset($registerTipoEnsinos))
                            {{$registerTipoEnsinos}}
                        @endif</label>
                    <form action="{{route('painel.estudantes.filter.entradas.tipo.ensino', [\Carbon\Carbon::now('America/Sao_Paulo')->monthName,  \Carbon\Carbon::now('America/Sao_Paulo')->day])}}"
                          method="GET">
                    ...
                    <table class="table table-sm table-hover table-bordered" id="lista">
                        ...
                        @foreach($entrances as $entrance)
                            <tr style="background-color:#ffffff !important;">
                                <td class="text-center">{{$entrance->student->room->name}}</td>
                                <td>{{$entrance->student->name}}</td>
                                <td class="text-center">{{\Carbon\Carbon::parse($entrance->updated_at)->format('d')}}</td>
                                <td class="text-center">{{\Carbon\Carbon::parse($entrance->updated_at)->format('H:i')}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
    ...
    <script type="text/javascript">
        ...
        scanner.addListener('scan', function (content) {
            ...
            $(function () {
                ...
                document.getElementById('student_number_ra').value = resultado[5]; // Retorna o som em mp3
                let soundStore = new Howl({src: "{{asset('/sounds/beep-entrance.mp3')}}"});
                soundStore.play();
...
@push('scripts')
    ...
    {{--                // Ret0rna o beep em mp3--}}
    {{--                let soundStore = new Howl({--}}
    {{--                    src: "{{asset('/sounds/beep-entrance.mp3')}}",--}}
    {{--                });--}}
    {{--                soundStore.play();--}}
    {{--                // Refresh somente a tabela--}}
