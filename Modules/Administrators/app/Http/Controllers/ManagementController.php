...
class ManagementController extends Controller
{
    ...
    {
        $managements = (new User())->where('role', '=', SupportStatus::MA)->orderBy('name', 'asc')->get();

        return view('administrators::managements.index', compact('managements'));
    }
    ...
    public function import(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        if ($request->file('select_file') !== null) {
            Excel::import(new ManagementsImport(), $request->file('select_file'));
    ...
    {
        $management = (new User())->find($id);

        $roles = DB::table('roles')->select('id', 'name')->orderBy('name', 'asc')->get();

        return view('administrators::managements.edit', compact('management', 'roles'));
    ...
    public function updateRole(Request $request, int $id): RedirectResponse
    {
        ...
        $update = (new User())->where('id', $management->id)->update([
            'role' => $request['role'],
        ]);
    ...
    public function updateAvatar(Request $request, int $id): RedirectResponse
    {
        ...
        if ($request['avatar'] !== null) {
            $image_name = Str::slug($management->name, '-') . '.' . explode(
                    '/',
            ...
            $management_image = public_path('/assets/uploads/images/users/');
            if (file_exists($management_image)) {
                @unlink($management_image);
            }
        }
    ...
    public function delete(int $id): RedirectResponse
    {
        ...
        $inactive = (new User())->where('id', $management->id)->delete();
    ...
    {
        $managements = (new User())::onlyTrashed()->paginate(50);

        return view('administrators::managements.inativos.index', compact('managements'));
    }
    ...
    {
        $management = (new User())::onlyTrashed()->find($id);

        $restore = (new User())->where('id', $management->id)->restore();
    ...
    public function forceDelete(int $id): RedirectResponse
    {
        ...
        $inactive = (new User())->where('id', $management->id)->forceDelete();
